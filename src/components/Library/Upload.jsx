...
const Upload = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/categories`, { headers }).then(r => r.json()).then(d => setCategories(Array.isArray(d) ? d : [])).catch(() => setCategories([]))
    fetch(`${API_BASE}/classes`,    { headers }).then(r => r.json()).then(d => setClasses(Array.isArray(d) ? d : [])).catch(() => setClasses([]))
  ...
  const handleSubmit = async (e, publish) => {
    ...
    if (formData.isPremium && (!formData.price || Number(formData.price) <= 0)) {
      setError('Please enter a price greater than 0 for premium resources.')
      return
    }
    ...
    try {
      ...
      body.append('isPremium',      String(formData.isPremium))
      body.append('price',          formData.isPremium ? String(formData.price) : '0')
      ...
      <div className="bg-[#161b22] border border-[#21262d] rounded-xl overflow-hidden">
        ...
        <div className="p-6">
          <form onSubmit={e => e.preventDefault()}>
            ...
            {/* Visibility */}
            <div className="mb-4">
              <label className="text-xs font-medium text-[#8b949e] uppercase tracking-wider mb-1.5 block">Visibility</label>
                ...
                <button type="button" disabled={loading}
                  ...
                  className={`flex-1 py-2 text-sm font-medium transition ${
                    !formData.isPremium
                      ...
                      : 'bg-[#1c2330] text-[#8b949e] hover:text-[#e6edf3]'
                  }`}>
                  🆓 Free
                </button>
                <button type="button" disabled={loading}
            ...
            {/* Price input – only shown when Premium is selected */}
            {formData.isPremium && (
              <div className="mb-4">
                <label className="text-xs font-medium text-[#8b949e] uppercase tracking-wider mb-1.5 block">
                  Price (MWK) *
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-[#8b949e]">MWK</span>
                  <input
                    type="number"
                    name="price"
                    ...
                    step="1"
                    placeholder="e.g. 500"
                    className="w-full bg-[#1c2330] border border-[#2563eb] rounded-lg pl-14 pr-3 py-2 text-sm text-[#e6edf3] outline-none focus:border-[#388bfd] disabled:opacity-50"
                  />
                </div>
                <p className="text-[11px] text-[#6e7681] mt-1">Students will need to pay this amount to access the resource.</p>
              </div>
            )}
            {/* ──────────────────────────────────────────────────────── */}
